...
func WaitForLogEntries(timeoutMs int64) []LogEntry {
	...
	defer func() {
		...
		// Remove by identity to avoid stale indexes under concurrent activity.
		for i, ch := range logNotifyChannels {
			if ch == notifyCh {
				logNotifyChannels = append(logNotifyChannels[:i], logNotifyChannels[i+1:]...)
				break
			}
		}
	}()
...
func (w *bufferWriter) Write(p []byte) (int, error) {
	...
	// Keep raw line so consumers (e.g., Python) can parse JSON fields/levels.
